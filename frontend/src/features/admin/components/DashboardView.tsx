...
export const DashboardView = ({ messages, stats, activeTab, setActiveTab, error }: DashboardViewProps) => (
  <div className="max-w-6xl mx-auto p-6 md:p-12 animate-in slide-in-from-bottom-4 duration-500">
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
      ...
      <div className="flex p-1 bg-slate-900/80 rounded-lg border border-slate-800">
        {(['messages', 'analytics'] as const).map((tab) => (
          ...
          >
            {tab === 'messages' ? `Inquiries (${messages.length})` : 'Analytics'}
          </button>
        ))}
      </div>
    </div>
    ...
    <div className="min-h-[400px]">
      {activeTab === 'messages' ? <MessageList messages={messages} error={error} /> : <VisitorList stats={stats} />}
    </div>
  </div>
);
